...
URL_TO_CHECK = 'https://pro-api.coinmarketcap.com/v2/cryptocurrency/quotes/latest'
...
def get_response_crypto(crypto):
    ...
    try:
        ...
        price = (data.get('data').get(crypto)[0].get('quote').get('USD').get('price'))
        CryptoDataNotFoundError.check_condition(data)
    except (ConnectionError, Timeout, TooManyRedirects, CryptoDataNotFoundError) as e:
        return e
    return price
...
async def handle_start(message, state):
    ...
    await message.answer(text="Приветствую! Я бот который поможет отследить нужный курс крипты!", reply_markup=get_on_start_kb())


@dp.message(F.text == 'Отследить крипту')
async def handle_crypto(message, state):
    await state.set_state(ClientState.CRYPTO)
    await message.answer(text="Введите алфавитный код криптовалюты. Например 'BTC'")
...
async def handle_alf(message, state):
    ...
    try:
        ...
        info = f"{result.get('data').get(crypto)[0].get('quote').get('USD').get('price')}$ текущая цена за одну монету. Теперь установите минимальный, максимальный или оба порога для оповещения."
    except (CryptoDataNotFoundError) as e:
        await message.answer(text="Такой криптовалюты либо нет на бирже, либо неправильно введён код")
...
async def handle_min(message, state):
    ...
    await message.answer(text="Дробные числа необходимо писать в формате '10.10'")

@dp.message(and_f(F.text == 'Установить максимальный порог', ClientState.RATES))
...
async def handle_minmax(message, state):
    ...
    await message.answer(text="Напишите минимальный и максимальный порог через пробел по возрастанию, например '50000 25000000'")
    await message.answer(text="Дробные числа необходимо писать в формате '10.10'")


@dp.message(or_f(ClientState.MIN_MAX, ClientState.MAX_RATE, ClientState.MIN_RATE))
async def handle_porog(message, state):
    current_state = await state.get_state()
    if not re.match(r'^[0-9]+(?:\.[0-9]+)?$', message.text) and not current_state == ClientState.MIN_MAX:
        await message.answer(text="Неправильный ввод. Введите только целые или дробные числа")
    elif not re.match(r'^\d+(\.\d+)?\s\d+(\.\d+)?$', message.text) and current_state == ClientState.MIN_MAX:
        await message.answer(text="Неправильный ввод. Введите только целые или дробные числа через пробел.")
    ...
    while not condition_met:
        if await check_condition_crypto(crypto, min, max):
            ...
            if min:
                ...
                excepted = max
            text = f'Крипта достигла порогового значения {excepted}=>{get_response_crypto(crypto)}'
